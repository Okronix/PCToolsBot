# PC Tools Bot
PCToolsBot - это программа на Python, созданная для простого управления своим компьютером через telegram бота. Она позволяет делать скриншоты экрана, управлять мышкой, управлять файлами и процессами, выполнять команды через cmd, выключать или перезагружать компьютер, а также загружать и скачивать файлы на компьютер.

Для использования программы необходимо указать внутри кода API телеграм бота, а также свой telegram ID и перевести скрипт в исполняемый файл. Управлять компьютером можно только с того аккаунта, telegram id которого будет указан в коде.При попытке использования телеграм бота другим аккаунтам, вам придет оповещение, что кто-то другой попытался запустить бота для управления вашим компьютером через бота.
## Скриншоты
